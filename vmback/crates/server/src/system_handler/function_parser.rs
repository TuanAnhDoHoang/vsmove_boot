...
pub fn function_parse(module_code: &str, function_name: &str) -> Vec<String> {
    let iter = regex_captures_iter!(r"public\s+fun\s+([[:alpha:]_]\w*)\s*\(", module_code);
    let mut funs = vec![];
    for (_, [function_name]) in iter.map(|c| c.extract()) {
        funs.push(function_name.to_string());
    }
...
mod tests {
    ...
    use crate::system_handler::function_parser::{function_parse};
    #[test]
...
module 0x1eabed72c53feb3805120a081dc15963c204dc8d091542592abaf7a35689b2fb::acl {
    ...
    ";
        let funs = function_parse(module_code, "set_roles");
        assert!(funs.len() == 8);
    }
}
